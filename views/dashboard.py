...
if role == "UID":
    ...
    st.subheader("Radar Skor per Aspek (rata-rata unit yang ditampilkan)")
    agg_rows = []
    for _, row in scores_df.iterrows():
    ...
    if agg_rows:
        ...
        agg = combined.groupby("aspect_name", as_index=False)["avg_level"].mean()
        fig3 = px.line_polar(agg, r="avg_level", theta="aspect_name", line_close=True, range_r=[0, 5])
    ...
    with st.expander("Contoh perhitungan (klik untuk lihat detail per aspek suatu unit)"):
        ...
        st.dataframe(
            detail_df.rename(columns={
                ...
                "bobot_aspek": "Bobot Aspek", "avg_level": "Rata-rata Level",
            }),
...
# ---------------------------------------------------------------------------
    ...
    if not aspect_df.empty:
        fig = px.bar(aspect_df, x="aspect_name", y="avg_level", color="avg_level",
                     ...
                     labels={"aspect_name": "Aspek", "avg_level": "Rata-rata Level"})
        st.plotly_chart(fig, use_container_width=True)

        st.markdown("#### Rincian Kontribusi Nilai per Aspek")
        st.dataframe(
            aspect_df.rename(columns={
                "aspect_name": "Aspek", "aspect_score": "Kontribusi Nilai",
                "bobot_aspek": "Bobot Aspek", "avg_level": "Rata-rata Level",
            }),
            use_container_width=True, hide_index=True,
